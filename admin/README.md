...
- **Produits personnalisés** (`/custom-orders`) : file d'attente des demandes "pedido personalizado" filtrable par statut (articles avec lien produit/quantité/prix affiché/notes), confirmation (crée la commande réelle, prix calculé par le moteur de marge) ou rejet (motif obligatoire) — voir `docs/PLAN.md` §7duodecies ter.
