...
- **Sprint 4** (moteur de calcul de prix) : terminé — conversion EUR→MRU, précédence des marges catégorie > boutique > global (confirmée), grille de frais de livraison par tranche de prix, historique des taux. Le cas chiffré du CDC (29,95 € → 2 057,16 MRU) est vérifié au centime près. 83 tests verts au total.

Voir `docs/PLAN.md` section 7/7bis/7ter/7quater/7quinquies pour le détail de chaque sprint livré, et section 8 pour les points encore ouverts (granularité fine des permissions, opérateur SMS à choisir, règle de suppression boutique à finaliser au Sprint 6, connecteur de synchronisation réel et service de traduction à choisir avant mise en production) avant le Sprint 5.
