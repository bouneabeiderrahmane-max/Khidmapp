...
- **Admin React — connexion à l'API** (hors découpage initial) : terminé — les six pages (tableau de bord, boutiques, commandes, paiements, utilisateurs, rôles, rapports) sont désormais connectées aux 96 opérations d'API réelles, vérifiées en navigateur contre le backend réel. **Contenu de ces pages en français en dur, pas encore bilingue** (seul le squelette du Sprint 0 l'est) ; pas de commande manuelle ni de tests automatisés côté admin — voir `admin/README.md`.
