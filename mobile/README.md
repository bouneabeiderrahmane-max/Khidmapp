...
- **Accueil** (`/`, premier onglet) : accueil nominatif, recherche de boutique, grille des boutiques actives (`GET /boutiques`) — tapoter une boutique ouvre son vrai site dans un **navigateur intégré à l'application** (`BoutiqueWebViewPage`, `webview_flutter`) plutôt qu'un catalogue interne ou le navigateur externe du téléphone, puisque Khidmapp ne synchronise pas réellement de catalogue (voir `StubCatalogFetcher`) et que le client doit rester dans l'app. Une bannière apparaît en bas de l'écran dès qu'un montant plausible est détecté sur la page courante (script JS best-effort cherchant un mot-clé de total près d'un montant en euros — **heuristique, jamais garantie**, chaque site ayant sa propre structure) ; "Continuer avec Khidmapp" pré-remplit alors le formulaire de commande personnalisée (lien + prix détecté, à vérifier). Sans détection, le bouton flottant "Commande personnalisée" (formulaire vide) reste la solution de repli — Khidmapp n'achète jamais automatiquement et n'intercepte jamais le vrai paiement du site tiers. Bouton "Commande personnalisée" vers le formulaire multi-produits (lien produit + quantité + prix affiché + notes par article, adresse/paiement/poids de colis comme un checkout classique, `POST /custom-order-requests`) — le prix EUR saisi est immédiatement doublé d'un aperçu MRU marge incluse (`GET /custom-order-price-preview`, débattu 500 ms après la dernière frappe), pour que le client ne voie jamais un prix EUR seul. Raccourci vers mes demandes (`GET /custom-order-requests`, détail avec traceur des 5 étapes Révision → Achat → Réception/contrôle qualité Madrid → Expédition internationale → Livraison Nouakchott — voir `docs/PLAN.md` §7duodecies ter). Nécessite une connexion et au moins une adresse enregistrée pour soumettre une demande ; le prix final n'est connu qu'après confirmation par l'équipe Khidmapp (statut "En attente de validation" puis "Confirmée"/"Rejetée").
...
- **Détection automatique de prix best-effort, pas fiable à 100 %** : `_priceDetectionScript` (`BoutiqueWebViewPage`) cherche un mot-clé de prix (produit **ou** panier — volontairement large pour ne jamais exiger de connexion au site tiers) près d'un montant en euros dans le texte de la page, re-scanné toutes les 2 s pendant 20 s après chaque navigation pour couvrir les sites SPA (contenu chargé après coup, changement de page sans navigation complète). Heuristique générique, sans connaissance de la structure propre à chaque boutique (Amazon, Zara, Decathlon…), qui peut ne rien trouver, trouver un montant qui n'est pas le bon, ou casser silencieusement si le site change sa mise en page. Toujours présenté comme "détection automatique, à vérifier" — jamais comme un prix garanti — et le client reste libre de le corriger avant l'envoi.
