...
- **Détection automatique de prix best-effort, pas fiable à 100 %** : `_priceDetectionScript` (`BoutiqueWebViewPage`) cherche un mot-clé de total près d'un montant en euros dans le texte de la page — heuristique générique, sans connaissance de la structure propre à chaque boutique (Amazon, Zara, Decathlon…), qui peut ne rien trouver, trouver un montant qui n'est pas le bon (ex. le prix d'un article isolé plutôt que le total du panier), ou casser silencieusement si le site change sa mise en page. Toujours présenté comme "détection automatique, à vérifier" — jamais comme un prix garanti — et le client reste libre de le corriger avant l'envoi.
