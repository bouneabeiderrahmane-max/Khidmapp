...
- **Panier** (`/cart`) : contenu réel du panier (`GET /cart`), quantité modifiable/suppression par ligne (`PUT`/`DELETE /cart/items/{id}`), sous-total/frais de livraison/total, bouton vers le checkout.
- **Checkout** (`/checkout`) : choix de l'adresse et du mode de paiement (Bankily/virement manuel), création de la commande réelle (`POST /orders` — vide le panier côté API), initiation Bankily (`POST /orders/{id}/payments/bankily/initiate`, gateway placeholder `StubBankilyGateway` — confirmation réelle async par webhook, non simulable ici).
- **Commandes** (`/orders`, `/orders/:id`) : liste (`GET /orders`), détail (articles, paiements, historique des statuts), auto-annulation dans la fenêtre gratuite (`POST /orders/{id}/cancel`) — la validation de la fenêtre reste entièrement côté API, l'écran relaie tel quel le message d'erreur 422 si le refus est trop tardif.
...
- **Aucune conversion de prix sur le vrai site d'une boutique** : le clic ouvre le site externe réel (navigateur du téléphone, hors de l'app) — techniquement impossible et trompeur d'y réécrire les prix EUR affichés en MRU (ce serait un prix fabriqué superposé au vrai site d'un tiers). L'aperçu MRU (marge incluse) reste disponible uniquement dans les écrans que Khidmapp contrôle, au moment où le client saisit le prix qu'il vient de lire (formulaire de commande personnalisée).
