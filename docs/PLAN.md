...
- **Détection possible, jamais garantie** : `_priceDetectionScript` (JS injecté via `WebViewController.runJavaScript`, communication par `JavaScriptChannel`) cherche un mot-clé de total (total/subtotal/sous-total/montant/importe/suma) proche d'un montant en euros dans le texte de la page, environ 1,2 s après le chargement (pour laisser le temps aux panes JS de type SPA de se peupler). Purement heuristique — aucune connaissance de la structure propre à chaque boutique, contrairement à une app mature dédiée à ce marché.
