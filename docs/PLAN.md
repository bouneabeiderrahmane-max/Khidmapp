...
Retour du client sur le choix du §7duodecies ter (site réel de la boutique ouvert dans le navigateur externe du téléphone) : "dans les autres apps, tout reste dans l'app" — le client ne veut pas être sorti de Khidmapp, même vers son propre navigateur. Tranché : le site reste ouvert **dans un navigateur intégré à l'application** (`BoutiqueWebViewPage`, `webview_flutter`) plutôt que dans le navigateur externe — le raisonnement de fond ne change pas (toujours le vrai site du tiers, toujours impossible/trompeur d'y réécrire un prix MRU par-dessus), seul le conteneur change. `url_launcher` (devenu inutilisé) et l'entrée `<queries>` associée dans `AndroidManifest.xml` ont été retirés ; au passage, la permission `INTERNET` — absente du manifest principal, présente seulement en debug/profile par défaut Flutter — a été ajoutée explicitement, sans quoi un build release n'aurait pu faire aucun appel réseau.

---

## 7duodecies septies. Détection best-effort de prix dans le navigateur intégré (extension hors CDC)

Le client a fourni des captures d'écran d'une app tierce de référence explicitement nommée ("Banango") : une fois une boutique ouverte, elle détecte automatiquement le contenu du panier réel du site tiers, affiche une bannière avec son propre montant et ses propres frais par-dessus la page, puis fait basculer l'achat vers son propre système de paiement. Demande confirmée explicitement par le client malgré les limites signalées avant implémentation :
- **Détection possible, jamais garantie** : `_priceDetectionScript` (JS injecté via `WebViewController.runJavaScript`, communication par `JavaScriptChannel`) cherche un mot-clé de prix — volontairement large (price/precio/prix pour une fiche produit **et** total/subtotal/montant/importe pour un panier, afin de fonctionner sans jamais exiger que le client se connecte au site tiers pour atteindre son panier) — proche d'un montant en euros dans le texte de la page. Re-scanné toutes les 2 s pendant 20 s après chaque navigation (pas une seule fois) : beaucoup de sites e-commerce sont des SPA dont le contenu se charge après l'évènement "page terminée", ou changent de "page" (produit → panier) sans déclencher de navigation complète détectable par le WebView. Purement heuristique — aucune connaissance de la structure propre à chaque boutique, contrairement à une app mature dédiée à ce marché.
- **Aucune interception réelle du paiement du tiers** : contrairement à l'app de référence, Khidmapp ne bascule jamais automatiquement l'achat vers son propre système — il n'existe techniquement aucun moyen de le faire depuis une WebView sur un site qu'on ne contrôle pas. La bannière ("Continuer avec Khidmapp") pré-remplit simplement le formulaire de "commande personnalisée" existant (lien de la page courante + montant détecté) : le client vérifie puis envoie, comme n'importe quelle demande personnalisée — Khidmapp achète toujours manuellement après confirmation admin (voir §7duodecies ter), jamais automatiquement.
- **Repli explicite** : sans détection, le bouton flottant "Commande personnalisée" (formulaire vide, comme avant) reste disponible — aucune fausse détection n'est jamais affichée comme certaine.

**Implémentation** : `BoutiqueWebViewPage` (script + bannière + navigation pré-remplie) ; `NewCustomOrderPage` accepte désormais `initialProductUrl`/`initialPriceEur` (route `/custom-order/new` avec un `extra` record `({productUrl, priceEur})`), qui déclenchent aussi l'aperçu MRU immédiatement comme une saisie manuelle.
